...
def attention_layer(attention_size, inputs, name, sparse=False):
    w_omega = tf.get_variable('w_omega_'+name, initializer=tf.random_normal([inputs.shape[-1], attention_size]))
...
class RegAttention(Model):
    def __init__(self, batch_size=10, max_len=30, lstm_size=20, vocab_size=10000, embeddings_dim=20, keep_probs=0.9, attention_size=16, use_embedding=True, reg="none", lam=None, sparse=False):
    ...
    def build_model(self):
        ...
        self.logits = dense_layer(last_output, 2, activation=None)
...
class LSTMPredModelWithMLPKeyWordModelAdvTrain(Model):
    ...
    def build_model(self):
        ...
        self.train_op = self.optimizer.minimize(self.cost + adv_cost)
...
class LSTMPredModelWithRegAttentionKeyWordModelHEX(Model):
    ...
    def build_model(self):
        ...
        reg = get_reg(self.kwm_alphas, lam=self.lam, type="entropy")
